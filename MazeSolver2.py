...
robot = Create3(Bluetooth("AUDREY-BOT"))  # Will connect to the first robot found.
...
def getNavigableNeighbors(wallsAroundCell, potentialNeighbors, prevCell, nXCells, nYCells):
    ...
    # Check left, front, right directions
    ...
    if prevCell and isValidCell(prevCell, nXCells, nYCells):
...
async def navigateMaze(robot):
    ...
    while HAS_COLLIDED == False:
        ...
        print(f"IR values: {IR0=}, {IR3=}, {IR6=}")
        print(f"Wall configuration: {walls}")
        print(f"At {CURR_CELL}, heading: {heading}, orientation: {orientation}")
